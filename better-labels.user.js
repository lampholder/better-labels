...
const ElementFactory = {
    ...
    listLabel: (label) => {
        ...
        var name = label.fields.displayName || label.name;
        var description = label.fields.description || '';
        var labelHTML = '<a class="d-inline-block IssueLabel v-align-text-top labelstyle-fc2929 linked-labelstyle-fc2929" ' +
            'style="background-color: ' + bgcolor + '; color: ' + fgcolor + '" title="' + description + '" ' +
            'href="/vector-im/riot-web/issues?q=is%3Aissue+is%3Aopen+label%3Abug">' + name + '</a>';
        return jQuery(labelHTML);
    }
};
...
const IssuePage = {
    processPage: (win) => {
        var issue = win.location.pathname;
...
const IssueList = {
    processPage: (win) => {
        ...
        jQuery('.js-issue-row').each((index, row) => {
            ...
            label_holder.empty();
            LabelsAPI.getLabelsForIssue(window.location.pathname + '/' + id)
            .then((labels) => {
                labels.forEach((label) => {
                    label_holder.append(ElementFactory.listLabel(label));
                    label_holder.append(document.createTextNode(' '));
                });
            });
        });
    }
